...
export const CurrentAffairsView: React.FC = () => {
  ...
  const filteredMcqs = useMemo(() => {
    return allCurrentMcqs.filter((mcq) => {
      ...
      if (mcqScope === 'Pakistan' && !PAKISTAN_CURRENT_AFFAIRS_MCQS.some((item) => item.id === mcq.id)) return false;
      if (mcqScope === 'World' && !WORLD_CURRENT_AFFAIRS_MCQS.some((item) => item.id === mcq.id)) return false;
  ...
  }, [allCurrentMcqs, mcqScope, subtopicFilter, searchQuery, sourceFilter]);
  ...
  useEffect(() => { setPage(1); }, [mcqScope, sourceFilter, subtopicFilter, searchQuery]);
